...
### `questlog:random`
Rolls one of its entries at random, weighted. Unlike `questlog:choice` the player does not pick
— the roll happens when the reward is claimed, and the outcome is saved, so it stays the same
across restarts.

#### Properties
- `entries`: A list of `{ "weight": number, "reward": Reward }` objects. `weight` defaults to 1.
- `empty_weight` (optional, default: 0): The weight of rolling nothing at all.
- `roll_count` (optional, default: 1): How many times to roll. Each roll is independent, so the
  same entry can come up more than once.

```json
{
  "type": "questlog:random",
  "empty_weight": 1.0,
  "entries": [
    { "weight": 2.0, "reward": { "type": "questlog:item", "item": "minecraft:diamond", "count": 2 } },
    { "weight": 5.0, "reward": { "type": "questlog:item", "item": "minecraft:emerald" } },
    { "weight": 1.0, "reward": { "type": "questlog:experience", "experience": 100 } }
  ]
}
```
