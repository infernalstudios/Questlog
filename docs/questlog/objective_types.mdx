...
- `components` (optional): Data component matching rules (e.g., `"minecraft:custom_name"`).
...
- `item`: The item to track.
May be the item itself or a tag prefixed with `#`.
- `slot` (optional): The equipment slot to track (`mainhand`, `offhand`, `head`, `chest`, `legs`, `feet`). If omitted, any slot is checked.
- `components` (optional): Data component matching rules (e.g., `"minecraft:custom_name"`).
...
- `item`: The item to track.
May be the item itself or a tag prefixed with `#`. May also be an object containing `id` and `components`.
- `components` (optional): Data component matching rules (e.g., `"minecraft:custom_name"`).
...
```json
{
  ...
  "components": {
    "minecraft:custom_name": "'Magic Feather'"
  },
  "required_amount": 1
}
```
