...
- `trackSinceStart` (optional, default: true):<br>
...
Note: Does not work on Fabric yet
...
- `objectives`: A list of objective definitions. The quest logic will check each of these; if one is complete, the `or` objective is complete.

### `questlog:not`
Completes if the child objective is **not** completed. This acts as an inverter.
